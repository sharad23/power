...
@section('content')
 ...
 <section class="content-header">
          ...
          <ol class="breadcrumb">
            ...
            <li class="active"><a href="{{URL::to('/pod_ups')}}"> Pod Ups</a></li>
          </ol>
        </section>
    ...
    <div class="row">
        <div class="col-xs-12">
            <div class="box">
				...
				{{ Form::open(array('url' =>'add-pod-ups/'.$pod_inventory->id, 'method' => 'post','id' =>'test')) }}
					
					@if(empty($pod_ups))					
						<div id="ups_detail" class="box-body ups_detail">		
						<button type="button" class="btn btn-default removeButton"><i class="fa fa-minus"></i></button>													
						   <div class="form-group">
								{{ Form::label('brand', 'Ups Brand') }}
						...
						{{ Form::hidden('ups_id[]', 0) }}							
						</div>
						
					 
					@else
						@foreach($pod_ups as $podups)
												
						<div id="ups_detail" class="box-body ups_detail">
						{{ Form::hidden('ups_id[]', $podups['id']) }}
						<button type="button" class="btn btn-default removeButton"><i class="fa fa-minus"></i></button>							
						   
						   <div class="form-group">
								{{ Form::label('brand', 'Ups Brand') }}
								{{ Form::text('brand[]',$podups['brand'],array('class'=>'form-control'))}}
								<p class="errors">{{$errors->first('brand')}}</p>
							</div>

							<div class="form-group">
								{{ Form::label('capacity', 'Capacity') }}
								{{ Form::text('capacity[]',$podups['capacity'],array('class'=>'form-control')) }}
								<p class="errors">{{$errors->first('capacity')}}</p>
							</div>
							...
							{{ Form::label('installed_date', 'Install Date') }}
								<div class='input-group date datepicker_recurring_start'>
									{{ Form::text('installed_date[]',$podups['installed_date'],array('class'=>'form-control')) }}
									<span class="input-group-addon">
										<span class="glyphicon glyphicon-calendar"></span>
									</span>
								</div>
